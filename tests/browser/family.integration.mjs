...
for (const [device, viewport] of [['desktop', { width: 1280, height: 900 }], ['mobile', { width: 390, height: 844 }]]) {
  test(`jornada real ${device}: RSVP, fraldas, mimos, sincronização e acessibilidade`, async () => {
    ...
    try {
      ...
      await expect(p.getByText('Sua reserva 2 pacotes')).toBeVisible()
      await expect(p.getByText('4 de 6 disponíveis')).toBeVisible()
      ...
      await expect(gift.getByText('Sua reserva 5 unidades')).toBeVisible()
      ...
      await expect(gift.getByText('Sua reserva 7 unidades')).toBeVisible({ timeout: 7500 })
...
for (const [device, viewport] of [['desktop', { width: 1280, height: 900 }], ['mobile', { width: 390, height: 844 }]]) {
  test(`compra informada ${device}: quantidade e troca somem, cancelar permanece`, async () => {
    ...
    try {
      ...
      await expect(m.getByText('Sua reserva 1 pacote')).toBeVisible()
      await m.getByRole('button', { name: 'Trocar tamanho' }).click()
      ...
      await expect(p.getByRole('button', { name: 'Trocar tamanho' })).toHaveCount(0)
...
test('M5: 320 px, texto a 200%, teclado e acessibilidade na presença e nos presentes', async () => {
  ...
  try {
    ...
    // Presença é uma seção fixa; Fraldas e Mimos são abas dentro de Presentes.
    assert.equal(await page.evaluate(() => document.documentElement.scrollWidth <= innerWidth), true, 'sem rolagem horizontal na presença')
    for (const name of ['Fraldas', 'Mimos']) {
