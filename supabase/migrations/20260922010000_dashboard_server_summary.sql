-- T-B7: projeções aditivas; respostas e reservas permanecem após revogar acesso.
-- Uma regra de saldo compartilhada pelo painel e pelo snapshot do convidado.
create function private.item_available(p_limit integer, p_committed bigint)
returns bigint language sql immutable set search_path='' as $$
  select case when p_limit is null then null else greatest(0::bigint,p_limit::bigint-p_committed) end;
$$;
revoke all on function private.item_available(integer,bigint) from public,anon,authenticated,service_role;

-- Summary, itens e reservas são lidos no mesmo comando/snapshot.
