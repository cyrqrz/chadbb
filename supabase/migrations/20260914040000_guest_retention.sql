-- Registros antigos recebem a data desta migration, sem supor uma idade desconhecida.
alter table private.guest_requests add column created_at timestamptz not null default now();
create index guest_requests_created_idx on private.guest_requests(created_at);
create index guest_sessions_expiry_idx on private.guest_sessions(expires_at);
create function private.cleanup_guest_data() returns void
language sql security definer set search_path='' as $$
  delete from private.guest_requests where created_at < now()-interval '90 days';
  delete from private.guest_sessions where expires_at <= now();
$$;
revoke all on function private.cleanup_guest_data() from public,anon,authenticated,service_role;
-- A stack Supabase fornece pg_cron; o PostgreSQL portátil não tem essa extensão.
do $$ begin
  if exists(select 1 from pg_available_extensions where name='pg_cron') then
    create extension if not exists pg_cron;
    perform cron.schedule('guest-data-retention','17 3 * * *','select private.cleanup_guest_data()');
  end if;
end $$;
