...
export function HomePage() {
  return <>
    <section className="page grid gap-12 py-16 md:grid-cols-[1.2fr_1fr] md:py-24">
      <div>
        <p className="mb-5 text-sm font-semibold uppercase tracking-widest text-brand">Um novo capítulo vem aí</p>
      ...
      <aside className="flex flex-col justify-center rounded-panel border border-line bg-brand-soft shadow-sm p-8 md:p-10">
        <span aria-hidden="true" className="mb-8 text-5xl text-brand">✳</span>
        ...
        <p role="status" className="mt-8 border-t border-line pt-5 text-sm text-stone-700">{backend.status === 'ready' ? 'Crie seu evento e monte a lista de presentes pelo menu acima. Convites estão em preparação.' : backend.message}</p>
      </aside>
    </section>
    <section id="como-funciona" className="pb-20">
      ...
      <div className="stagger grid gap-8 md:grid-cols-3">{steps.map(([number, title, description]) =>
        <article key={number} className="border-t border-stone-300 pt-5"><span className="text-sm text-brand">{number}</span><h3 className="mt-4 text-lg font-semibold">{title}</h3><p className="mt-2 leading-relaxed text-stone-600">{description}</p></article>,
      )}</div>
    </section>
  </>
}
