...
function EventEditor({ server, refreshing }: { server: EventRecord; refreshing: boolean }) {
  ...
  return <section className="page max-w-3xl">
    ...
    <nav aria-label="Áreas do evento" className="mt-6 flex flex-wrap gap-3"><Link to={`/eventos/${record.id}/convites`} className="secondary">Convites e confirmações →</Link><Link to={`/eventos/${record.id}/presentes`} className="secondary">Lista de presentes →</Link></nav>
