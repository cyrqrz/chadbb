...
| `set_event_step` | `p_event_id`, `p_version`, `p_step` (`guests` ou `gifts`), `p_done` booleano | Proprietário, evento publicado; grava ou limpa `guests_done_at`/`gifts_done_at` (marcar de novo mantém a data) e retorna evento com nova versão | `EVENT_NOT_FOUND`, `INVALID_PAYLOAD`, `EVENT_PURGED`, `EVENT_CLOSED`, `EVENT_NOT_PUBLISHED`, `EVENT_RETENTION_DUE`, `VERSION_CONFLICT` |
