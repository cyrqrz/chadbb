...
- [ ] Executar verificação automatizada de acessibilidade e inspeção manual com teclado e leitor de tela; corrigir problemas que impeçam concluir a jornada. _(Verificação automatizada no `test:e2e` (axe em 7 specs). Teclado feito em 23/09: auditoria das telas do convite e do organizador, em desktop e celular, sem barreira — todo elemento focável tem indicador visível e nenhum fica atrás da barra de etapas; as jornadas do convidado e do organizador têm testes só com teclado (`tests/e2e/keyboard.ts`). Falta a inspeção manual com leitor de tela, com o titular.)_
...
- [ ] Concluir ensaio de concorrência, atualização e acessibilidade no ambiente do evento; registrar falhas corrigidas e limites remanescentes. _(Parcial em 23/09: concorrência e atualização medidas no `chadbb-cha` na T-B5 (362 POSTs, zero erros, sincronização abaixo de 7 s); fluxo do convidado, revogação e acesso cruzado no smoke da T-B4. Acessibilidade das páginas públicas publicadas — início, entrar, convite inválido/incompleto, 404 — sem violações de axe, sem estouro de largura a 320 px e com texto a 200%, e com indicador de foco em tudo, em desktop e celular. Falta passar convite e painel reais publicados, o que exige dados no `chadbb-cha` — ver `reviews/2026-09-23-adiantamento.md`.)_
